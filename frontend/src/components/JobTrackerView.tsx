...
export default function JobTrackerView({ initialJobId = "AE-2024-8901" }: JobTrackerViewProps) {
  const [searchQuery, setSearchQuery] = useState<string>(initialJobId);
  ...
  const handleLookup = async (idToSearch?: string) => {
    ...
    if (!term) {
      toast.error("Please enter a Job ID (e.g., AE-2024-8901) or phone number.");
      return;
    }
    ...
    try {
      ...
      setCurrentJob(data);
      toast.success(`Loaded Job #${data.job_id}`);
    } catch (err: any) {
      console.error(err);
      setCurrentJob(null);
      toast.error(`No record found for "${term}". Try sample ID: AE-2024-8901`);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-left max-w-3xl mb-10">
          ...
          <h2 className="font-heading font-black text-3xl sm:text-5xl uppercase text-white tracking-tight">
            Real-Time Equipment Repair & Service Status Tracker
          </h2>
          <p className="text-zinc-300 text-sm sm:text-base font-sans mt-2">
            Track your motor, pump, or panel repair stage in real-time. View laboratory Megger test readings, dynamic vibration balancing results, and estimated dispatch dates.
          </p>
        </div>
        ...
        <div className="bg-[#141414] border border-white/10 rounded-md p-6 max-w-4xl space-y-4 text-left">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Enter Job ID (e.g. AE-2024-8901) or Registered Phone"
                value={searchQuery}
        ...
        {currentJob && (
          <div className="mt-8 bg-[#121212] border-2 border-amber-500/40 rounded-md p-6 sm:p-8 space-y-8 electric-glow text-left animate-in fade-in-50 duration-300" data-testid="job-details-container">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 border-b border-white/10 pb-6">
              ...
              <div className="lg:col-span-8 space-y-2">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-xs font-mono text-zinc-400">
                    Warranty: {currentJob.warranty_months} Months
                  </span>
                </div>
                ...
                <p className="text-xs sm:text-sm font-mono text-zinc-300">
                  {currentJob.equipment_specs}
                </p>
                <div className="text-xs text-zinc-400 font-sans pt-1">
                  Customer: <strong className="text-zinc-200">{currentJob.customer_name}</strong> {currentJob.company_name && `(${currentJob.company_name})`}
                </div>
              </div>

              {/* Progress percentage & Dates */}
              <div className="lg:col-span-4 bg-[#181818] border border-white/10 rounded p-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px] font-mono pt-1 text-zinc-400">
                  <div>
                    <span className="block text-zinc-500">Intake Date</span>
                    <strong className="text-zinc-200">{currentJob.intake_date}</strong>
                  </div>
                  <div>
                    <span className="block text-zinc-500">Est. Dispatch</span>
                ...
                {currentJob.steps.map((step) => (
                  ...
                  >
                    <div className="flex items-center justify-between font-mono text-xs mb-1.5">
                      <span className="text-zinc-400 font-bold">Stage 0{step.step_number}</span>
                      {step.completed ? (
                    ...
                    <p className="text-[11px] font-sans text-zinc-400 mt-1 line-clamp-3">
                      {step.description}
                    </p>
                    {step.completed_at && (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left font-mono text-xs border border-white/10 rounded">
                    <thead className="bg-[#181818] text-zinc-400 uppercase text-[11px]">
                      <tr>
                        <th className="p-3 border-b border-white/10">Test Parameter</th>
                        <th className="p-3 border-b border-white/10">Observed Value</th>
                        <th className="p-3 border-b border-white/10">Standard Spec</th>
            ...
            {/* Technician Notes from Mohammad Afjal */}
            {currentJob.technician_notes && (
              <div className="bg-amber-950/20 border border-amber-500/30 rounded p-4 text-xs font-sans text-zinc-300">
                <div className="flex items-center gap-2 text-amber-400 font-heading font-bold uppercase text-sm mb-1">
                  ...
                  Technician Inspection Notes (Mohammad Afjal):
                </div>
                <p className="text-zinc-300 italic">
                  "{currentJob.technician_notes}"
                </p>
                <div className="mt-2 text-[11px] font-mono text-zinc-500">
                  Wire Grade: {currentJob.wire_type}
                </div>
              </div>
            )}
        ...
        {hasSearched && !currentJob && !isLoading && (
          <div className="mt-8 bg-[#141414] border border-white/10 rounded p-8 text-center max-w-xl mx-auto space-y-3">
            <AlertCircle className="w-8 h-8 text-amber-400 mx-auto" />
            <h4 className="font-heading font-black text-xl text-white uppercase">No Active Job Found</h4>
            <p className="text-xs text-zinc-400">
              We couldn't find a record for the entered query. Please verify the Job ID (e.g. <code>AE-2024-8901</code>) or call our workshop directly at <a href="tel:+919669718100" className="text-amber-400 font-bold">+91 9669718100</a>.
            </p>
          </div>
        )}
