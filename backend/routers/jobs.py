...
async def advance_job_stage(job_id: str, _: bool = Depends(require_owner)):
    ...
    pending_indices = [i for i, s in enumerate(steps) if not s.get("completed")]
    ...
    await db.job_trackers.update_one(
        {"job_id": doc["job_id"]},
        {"$set": {"steps": steps, "status_percentage": new_pct, "current_stage": new_stage, "updated_at": datetime.now(timezone.utc)}}
    )
    updated = await db.job_trackers.find_one({"job_id": doc["job_id"]})
    return StageAdvanceResult(job=JobTracker(**updated), message=f"'{completed_titles[-1]}' marked complete. Job is now at {new_pct}%.")

@router.patch("/{job_id}/notes", response_model=JobTracker)
async def update_job_notes(job_id: str, payload: JobNotesUpdate, _: bool = Depends(require_owner)) -> JobTracker:
    clean_id = job_id.strip()
    doc = await db.job_trackers.find_one({"job_id": {"$regex": f"^{clean_id}$", "$options": "i"}})
    if not doc:
        raise HTTPException(status_code=404, detail=f"No repair job found matching '{clean_id}'")
    notes = payload.technician_notes.strip()
    await db.job_trackers.update_one({"job_id": doc["job_id"]}, {"$set": {"technician_notes": notes, "updated_at": datetime.now(timezone.utc)}})
    doc["technician_notes"] = notes
    return JobTracker(**doc)

@router.post("", response_model=JobTracker)
async def create_job(job_in: JobTrackerCreate, _: bool = Depends(require_owner)):
    ...
    if not job_dict.get("steps"):
        ...
        job_dict["steps"] = [{"step_number": i + 1, "title": s[0], "description": s[1], "completed": s[2], "completed_at": datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M") if s[2] else None} for i, s in enumerate(stages)]
