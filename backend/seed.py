"""Seed the workshop demo data. Idempotent: clears the seeded collections first.
...
def _build_jobs() -> List[Dict[str, Any]]:
    """Three repair jobs at different stages of the 6-step workshop pipeline."""
    return [
        {
            "id": "job_001",
            "job_id": "AE-2024-8901",
            ...
            "steps": [
                {"step_number": 1, "title": "Diagnostic & Surge Testing", "description": "Equipment received. 5000V Megger test showed 0.2 MΩ (severe phase-to-ground short). Thermal burn pattern verified.", "completed": True, "completed_at": "2025-02-18 11:30"},
                {"step_number": 2, "title": "Stripping & Core Prep", "description": "Burnt winding stripped under controlled heat. Core slots cleared, deburred, and lined with Nomex Class-H insulation.", "completed": True, "completed_at": "2025-02-19 16:00"},
                {"step_number": 3, "title": "100% Copper Coil Winding", "description": "Hand-crafted 100% Electrolytic dual-coated copper winding installed. Phase separators and slot wedges fitted to spec.", "completed": True, "completed_at": "2025-02-20 18:45"},
                {"step_number": 4, "title": "Vacuum Pressure Varnish & Baking", "description": "Dual-dip high dielectric synthetic varnish, baked at 135°C in industrial oven for 8 hours for moisture barrier.", "completed": True, "completed_at": "2025-02-21 14:15"},
                {"step_number": 5, "title": "Dynamic Balancing & Load Testing", "description": "Rotor dynamic balancing (<1.2 mm/s vibration), new SKF 6310 C3 bearings fitted. Full load thermal run in progress.", "completed": True, "completed_at": "2025-02-22 10:00"},
                {"step_number": 6, "title": "Quality Certified & Ready", "description": "Final inspection certificate and dispatch clearance under Class-B supervisor.", "completed": False, "completed_at": None},
            ],
            "test_readings": [
...
def _build_reviews() -> List[Dict[str, Any]]:
    """Verified customer testimonials across mills, plants and irrigation."""
    return [
        {
...
def _build_emergency_dispatch() -> Dict[str, Any]:
    """One historical SOS ticket so the owner dashboard is never empty."""
    return {
        "id": "SOS-9812",
...
async def seed_jobs() -> None:
    ...
    await db.job_trackers.insert_many(_build_jobs())


async def seed_reviews() -> None:
    print("Seeding customer reviews...")
    await db.reviews.insert_many(_build_reviews())


async def seed_emergencies() -> None:
    print("Seeding sample emergency dispatch...")
    await db.emergency_dispatches.insert_one(_build_emergency_dispatch())
